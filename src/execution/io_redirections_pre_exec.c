...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   io_redirections_pre_exec.c                         :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: aldokezer <aldokezer@student.42.fr>        +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/14 17:16:48 by aldokezer         #+#    #+#             */
/*   Updated: 2024/05/14 17:52:01 by aldokezer        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_pre_input_redirection(char *file_name, int *fd_in)
{
...
void	ft_pre_output_redirection(char *file_name, int *fd_out)
{
	int	temp_fd;
...
void	ft_pre_append_redirection(char *file_name, int *fd_out)
{
	int	temp_fd;
...
void	ft_pre_process_redirection(t_token *token, int *fd_in, int *fd_out)
{
	if (token->type == R_INFILE)
	{
		if (*fd_in != STDIN_FILENO)
			close(*fd_in);
		ft_pre_input_redirection(token->text, fd_in);
	}
	else if (token->type == R_OUTFILE)
	{
		if (*fd_out != STDOUT_FILENO)
			close(*fd_out);
		ft_pre_output_redirection(token->text, fd_out);
	}
	else if (token->type == R_OUTFILE_APP)
	{
		if (*fd_out != STDOUT_FILENO)
			close(*fd_out);
		ft_pre_append_redirection(token->text, fd_out);
	}
}
...
void	ft_pre_redirect_io(t_cmd *cmd, int *fd_in, int *fd_out)
{
	...
	while (token)
	{
		ft_pre_process_redirection(token, fd_in, fd_out);
		token = token->next;
	}
	return ;
}
